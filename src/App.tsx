...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (savedAuth) {
      ...
      // Unlock app regardless of subscription for now
      setIsAppLocked(false);
  ...
  return (
    <div className={`fixed inset-0 h-full w-full flex flex-col overflow-hidden select-none p-2 transition-colors duration-300 ${classes.container}`}>
      ...
      {/* DESCRIPTION SIDEBAR */}
      <div className={`fixed inset-y-0 left-0 z-50 w-full sm:w-80 shadow-2xl border-r transition-transform duration-300 ease-in-out transform ${isDescriptionOpen ? 'translate-x-0' : '-translate-x-full'} ${classes.sidebar}`}>
        <div className="flex flex-col h-full p-4 sm:p-6 overflow-y-auto scrollbar-hide">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-lg sm:text-xl font-black uppercase tracking-tighter">Description</h2>
            <button onClick={() => setIsDescriptionOpen(false)} className="w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center rounded-full bg-gray-100 text-black text-xl font-bold active:scale-90 transition-transform">×</button>
          </div>
          ...
          <div className="space-y-5 flex-1">
            <div className="space-y-1.5">
              ...
              <input 
                ...
                className={`w-full p-3.5 sm:p-3 rounded-xl border outline-none text-sm font-bold ${classes.sidebarInput}`}
                placeholder="Titre"
              />
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex gap-2">
                <button 
                  ...
                  className={`flex-1 py-3 sm:py-2 rounded-lg text-xs font-bold border transition-all ${transactionType === 'achat' ? 'bg-emerald-500 text-white border-emerald-500' : 'bg-transparent opacity-40'}`}
                >ACHAT</button>
                <button 
                  onClick={() => setTransactionType('vente')}
                  className={`flex-1 py-3 sm:py-2 rounded-lg text-xs font-bold border transition-all ${transactionType === 'vente' ? 'bg-blue-500 text-white border-blue-500' : 'bg-transparent opacity-40'}`}
                >VENTE</button>
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="grid grid-cols-1 gap-1.5">
                <button 
                  ...
                  className={`w-full py-2.5 sm:py-2 rounded-lg text-[10px] font-bold border transition-all ${decimalHandling === 'all' ? 'bg-blue-500 text-white border-blue-500' : 'bg-transparent opacity-40'}`}
                >COMPTER LES VIRGULES</button>
                <button 
                  onClick={() => setDecimalHandling('none')}
                  className={`w-full py-2.5 sm:py-2 rounded-lg text-[10px] font-bold border transition-all ${decimalHandling === 'none' ? 'bg-blue-500 text-white border-blue-500' : 'bg-transparent opacity-40'}`}
                >NE PAS COMPTER</button>
                <button 
                  onClick={() => setDecimalHandling('threshold')}
                  className={`w-full py-2.5 sm:py-2 rounded-lg text-[10px] font-bold border transition-all ${decimalHandling === 'threshold' ? 'bg-blue-500 text-white border-blue-500' : 'bg-transparent opacity-40'}`}
            ...
            <div className="space-y-1.5 flex items-center gap-2 mt-2">
              ...
              <label htmlFor="includeBagPrice" className="text-[10px] font-bold uppercase opacity-50 cursor-pointer">
                Inclure le prix des sacs dans le total
              </label>
            </div>
            ...
            <div className="pt-4 border-t border-dashed mt-4">
              ...
              <div className="flex justify-between items-center mb-1">
                <span className="text-xs opacity-50">Total Sacs</span>
                <span className="font-bold">{globalTotalBags}</span>
              </div>
              <div className="flex justify-between items-center mb-1">
              ...
              {includeBagPrice && bagPrice > 0 && (
                <div className="flex justify-between items-center mt-2 pt-2 border-t border-dashed">
                  <span className="text-[10px] font-bold uppercase">Total avec sacs</span>
                  <span className="text-sm font-black text-emerald-500">
                    {formatCurrency((unitPrice * globalTotal) + (bagPrice * globalTotalBags))}
                  </span>
                </div>
              )}
            </div>
        ...
        <header className="flex flex-col gap-1 shrink-0 px-1 sm:px-0">
          <div className="flex gap-2 h-[7vh] min-h-[48px] max-h-[60px]">
              ...
              <div className="flex-1 flex flex-col min-w-0">
                ...
                {user && (
                  <span className="text-[8px] font-bold opacity-40 truncate -mt-1 uppercase tracking-tight">
                    {user.company_name || user.email.toLowerCase()}
                  </span>
                )}
              </div>
            </div>
            <div className="flex gap-1 sm:gap-1.5 shrink-0">
              {/* PAGE NAVIGATION */}
              <div className={`flex items-center gap-0.5 px-1 rounded-xl border ${classes.headerBg} border-opacity-50`}>
                <button 
                  onClick={() => setCurrentIndex(prev => Math.max(0, prev - 1))}
                  disabled={currentIndex === 0}
                  className={`w-7 h-full flex items-center justify-center text-xs transition-opacity ${currentIndex === 0 ? 'opacity-20' : 'opacity-100 active:scale-90'}`}
                >
                  <i className="fa-solid fa-chevron-left"></i>
                </button>
                <span className="text-[10px] font-black w-6 text-center opacity-50">{currentIndex + 1}/{pages.length}</span>
                <button 
                  onClick={() => setCurrentIndex(prev => Math.min(pages.length - 1, prev + 1))}
                  disabled={currentIndex === pages.length - 1}
                  className={`w-7 h-full flex items-center justify-center text-xs transition-opacity ${currentIndex === pages.length - 1 ? 'opacity-20' : 'opacity-100 active:scale-90'}`}
                >

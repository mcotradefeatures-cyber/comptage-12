        ...
        <div className={`mb-8 p-4 rounded-2xl border ${tableBorder} ${theme === 'night' ? 'bg-white/5' : 'bg-gray-50'}`}>
          ...
          <div className="flex flex-wrap items-end gap-4">
            <div className="space-y-1">
              <label className="text-[10px] font-bold uppercase opacity-50 ml-1">Prix Personnel (Ar)</label>
              <input 
                type="number" 
                value={personalPrice}
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-bold uppercase opacity-50 ml-1">Prix Équipe (Ar)</label>
              <input 
                type="number" 
                value={teamPrice}
            ...
            >
              {isUpdatingPrice ? 'Mise à jour...' : 'Enregistrer les prix'}
            </button>
          </div>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="text-[10px] font-bold uppercase opacity-50">
                  ...
                  <th className={`p-2 border-b ${tableBorder}`}>Société</th>
                ...
                {users.map((u) => {
                  ...
                  return (
                    <tr key={u.id} className="hover:bg-black/5 transition-colors">
                      <td className={`p-2 border-b ${tableBorder}`}>{u.email.toLowerCase()}</td>
                      <td className={`p-2 border-b ${tableBorder}`}>{u.company_name || '-'}</td>
                      ...
                      <td className={`p-2 border-b ${tableBorder}`}>
                        <select 
                          ...
                          className={`text-[10px] font-bold uppercase p-1 rounded outline-none ${selectClass} ${u.account_type === 'team' ? 'text-purple-500' : 'text-blue-500'}`}
                        >
